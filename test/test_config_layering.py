...
class TestConfigLayering(unittest.TestCase):
    def test_parse_success_nested_access(self) -> None:
        ...
        self.assertTrue(cfg.rag.hybrid_enabled)
        self.assertTrue(cfg.rag.reranker_enabled)

    def test_rag_invalid_mode_error_contains_key(self) -> None:
        raw = _base_raw_config()
        raw["rag"] = {"default_mode": "magic"}
        with self.assertRaisesRegex(ValueError, "rag\\.default_mode"):
